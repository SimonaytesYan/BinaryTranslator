...
В программном процессоре существовали только регистры `RAX`, `RBX`, `RCX` и `RDX`. Они соответствуют таким же регистрам в x86-64. Эти 4 регистры мы будем называть пользовательскими регистрами. Остальные регистры x86-64 - системными.

Гарантируется, что в начале программы все пользовательские регистры равны нулю.

Реализованна стандартная библиотека, включающая в себя реализации инструкций `in` и `out` программного процессора. Реализацию функций можно увидеть в [этом файле](Src/Stdlib/Stdlib.cpp). Функция `long long InputNumber10()` соответствует `in`, функция `void OutputNumber10(long long number)` соответствует `out`.\
Было принято решение написать собственную реализацию, вместо стандартных printf и scanf, из соображений производительности.

Компиляция осуществляется в два прохода, для обработки условных и безусловных переходов вниз.

Полученные команды помещаются в буфер для последующего исполнения (функция `int Translate(int* in_code, char* out_code, MyHeader* in_header, char* ram)` в [файле](Src/Translation/Translation.cpp)). После завершения компиляции с помощью ассемблерной вставки в функции `void Run(char* out_code)` (реализация в [этом файле](Src/Translation/Translation.cpp)) осуществляется переход на начало выше описанного буфера.
